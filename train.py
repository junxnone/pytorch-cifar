...
trainset = torchvision.datasets.ImageFolder(args.train_data_path, transform=transform_train)
...
def train(epoch):
    ...
    e.log_metric("accuracy", 1.0*correct/total)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
    e.log_metric("val_accuracy", 1.0*correct/total)


for epoch in range(start_epoch, start_epoch+args.epochs):
    print(f'\nEpoch/Total Epochs: {epoch}/{args.epochs}')
    train(epoch)
    test(epoch)
    scheduler.step()
